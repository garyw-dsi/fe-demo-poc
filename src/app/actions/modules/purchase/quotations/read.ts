...
type Quotations = components['schemas']['PaginatedQuotation'];
...
export const getAllQuotations = async ({ 
  ...
  try {
    ...
    const dummyData: Quotation = Array.from({ length: page_size }).map((_, index) => ({
      pk: index + 1,
      currency: {
        iso: "IDR",
    ...
    const dummyQuotations: Quotations = {
      items: dummyData,
      page: page,
...
type DetailQuotation = components['schemas']['Quotation'];
type QuotationItems = components['schemas']['QuotationItem'];
...
export const getQuotations = async ({ pk }: { pk: number }) => {
  try {
    ...
    const items: QuotationItems[] = Array.from({ length: 5 }).map(() => ({
      pk: faker.number.int({ min: 1, max: 100 }),
      price: Number(faker.finance.amount({ min: 50000, max: 1000000 })),
      discount_rate: Number(faker.number.float({ min: 0, max: 0.5 })),
      product: {
    ...
    const quotation: DetailQuotation = {
      pk: pk,
