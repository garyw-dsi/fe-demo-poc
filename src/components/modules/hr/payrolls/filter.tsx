...
import { payrollStatus } from "@/constants/modules/hr/payroll";

const PARAMETER_FILTERS = {
  status: "status",
};

export default function ModuleHRPayrollFilter() {
            ...
            <MenuGroup title="Status">
              ...
              >
                <Stack px={5} spacing={2}>
                  {payrollStatus.map((payroll, index) => (
                    <Radio key={index} value={payroll} size="sm">
                      {payroll}
                    </Radio>
                  ))}
                </Stack>
