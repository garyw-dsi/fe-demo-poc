...
import { type EditCustomer } from "@/libs/yup/crm/customers";
...
import { initialEditAddressValue } from "@/components/modules/crm/customers/edit/form";

interface AddressFieldsProps {
  index: number;
  remove: (index: number) => void;
  setAddressDeleted: React.Dispatch<React.SetStateAction<number[]>>;
...
const AddressFields = ({
  ...
  const onRemove = () => {
    ...
    const addressPk = values.address?.[index]?.pk;
    if (addressPk) {
      setAddressDeleted((prev) => [...prev, addressPk]);
    }
  }
...
export default function ModuleCRMCustomerEditAddressForm({
  setAddressDeleted
}: {
  setAddressDeleted: React.Dispatch<React.SetStateAction<number[]>>;
}) {
        ...
        {({ push, remove }) => (
          <Stack spacing={5} divider={<Divider />}>
            ...
            <Button
              display="none"
              ref={addAddressRef}
              onClick={() => push(initialEditAddressValue)}
            />
          </Stack>
        )}
      </FieldArray>
