...
export default function ModuleAccountingVendorsTableView({
  datas
}: {
    ...
    <ITable>
      ...
      <ITableBody>
        {datas.map((data, index) => (
          <TR key={index}
            onClick={() => router.push(`/modules/accounting/vendors/detail/${data.pk}`)}
          >
            <TD>{data.number}</TD>
            ...
            <TD>
              <Flex align={'center'} gap={2}>
                <Avatar
                  size={'xs'}
                  rounded={'sm'}
                  src={data.vendor.image.url || undefined}
