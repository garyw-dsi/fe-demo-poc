...
const AddressFields = ({
  ...
  const {
    ...
    touched,
    values
  } = useFormikContext<CreateContact>();
      ...
      <Stack spacing={4}>
        ...
        {values.address?.[index]?.address_type && (
          <Fragment>
            ...
            >
              <ModuleInputLabel label="Contact Phone" />
              <InputGroup>
            ...
            >
              <ModuleInputLabel label="Contact Email" />
              <Field
                as={ModuleInput}
...
export default function ModuleContactAddressForm() {
  const addAddressRef = useRef<HTMLButtonElement>(null);
  const { values } = useFormikContext<CreateContact>();
