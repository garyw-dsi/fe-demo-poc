...
import { deliveryTerms, orderStatus, paymentTerms } from "@/constants/modules/sales";
...
export default function ModuleSalesOrderFilter() {
  ...
  const initialPaymentTerm = params.get(PARAMETER_FILTERS.paymentTerm);

  const [deliveryTerm, setDeliveryTerm] = useState<string | null>(initialDeliveryTerm);
  const [paymentTerm, setPaymentTerm] = useState<string | null>(initialPaymentTerm);
              ...
              >
                <Stack px={5} spacing={2}>
                  {orderStatus.map((order, index) => (
                    <Radio key={index} value={order} size="sm">
                      {order}
                    </Radio>
                  ))}
                </Stack>
