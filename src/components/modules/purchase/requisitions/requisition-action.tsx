...
const PurchaseRequisitionModal = ({ isOpen, onClose, onSubmit }: {
  isOpen: boolean,
  onClose: () => void,
  onSubmit: () => void
}) => {
    ...
    <Modal isOpen={isOpen} onClose={onClose} closeOnOverlayClick={false} closeOnEsc={true}>
      ...
      <ModalContent bg={useColorModeValue('white', 'gray.800')}>
        ...
        <ModalBody>
          ...
          >
            Do you want to create an quotation for this requisition?
          </Text>
        </ModalBody>
        <ModalFooter gap={3}>
          <Button
...
export default function ModulePurchaseRequisitionAction() {
  ...
  const onSubmit = async () => {
    onClose();
    router.push(`/modules/purchase/quotations/create/${pk}`);
  }
