...
import { type CreateRequisition, paymentN } from "@/libs/yup/purchase/requisition";
import ModulePurchaseRequisitionFormLayout from "../layout";

export default function ModulePurchaseRequisitionPaymentCreate({
  currency
}: {
  currency: {
    value: string;
  ...
  const { errors, touched, values } = useFormikContext<CreateRequisition>();

  return (
    <ModulePurchaseRequisitionFormLayout
      title="Requisition Payment"
    >
      <Stack spacing={5}>
        <Flex
          gap={5}
